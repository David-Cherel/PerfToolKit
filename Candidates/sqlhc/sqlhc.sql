REM  
REM $Header: tfa/src/v2/tfa_home/resources/sql/sqlhc.sql smuthuku_sqlhc_updates/1 2024/12/28 19:22:12 smuthuku Exp $
REM
...
REM   may be performing poorly or if in DB mode for a database
REM
REM   Inputs: A memory-resident SQL_ID (optional)
REM
REM NOTES
REM 
...
REM   2. SQL_ID of interest (optional).
REM
REM EXECUTION
...
REM    scharala    12/28/24 - Added Histograms(SQLHC027)
REM    scharala    11/16/24 - Added View Metadata
Rem                           from SRDC sql scripts
Rem    bvongray    10/28/24 - Bug 37223660 - remove sqlsessst
Rem    smuthuku    10/09/24 - SQLHC version update ER 37154785
REM    scharala    08/04/24 - Added DB mode
REM    scharala    07/06/24 - SQL Sets info and new menu
Rem    smuthuku    07/05/24 - SQLHC version update ER 36770788
REM    scharala    06/23/24 - Collect top AWR in last 7 days
REM    scharala    06/15/24 - Added sqlhc_fc.sql to "utl"
REM    scharala    06/09/24 - Change menu layout and links
REM    scharala    06/08/24 - Amend observations
REM    scharala    06/02/24 - Show Hidden underscore parameters
REM    scharala    04/11/24 - Print N/A for full_plan_hash_value in below 12c
REM    scharala    04/10/24 - Truncate the SQL Text in DBA_SQL_PLAN_BASELINES
REM    scharala    03/24/24 - Compare only Major version in Observation 006
Rem    scharala    03/09/24 - Deal with failed 10053 trace(SQLHC047)
REM    scharala    03/08/24 - Added Full PHV to reports(SQLHC035)
Rem    smuthuku    03/06/24 - SQLHC version update ER 36374176
REM    scharala    12/24/23 - Add pxhcdr report when PX query(SQLHC011)
REM    scharala    09/30/23 - Test for Tuning License and skip 
REM                           dbms_sqltune(SQLHC007)
REM    scharala    08/21/21 - Added log file for TCB facility
REM    scharala    08/05/21 - Added the TCB facility
REM    scharala    03/19/21 - Reduced messages relating to CP/COP MV/RENAME on
REM                           Windows / Linux
REM    scharala    03/05/21 - Fixed bugs relating to dbms_xplan and calling from TFA
REM    csierra     Created
...
DEF doc_ver = '23.2';
DEF doc_date = '2024/12/28';
-- sqldx_output: HTML/CSV/BOTH/NONE
DEF sqldx_output = 'CSV';

/**************************************************************************************************/

EXEC DBMS_APPLICATION_INFO.SET_MODULE(module_name => '^^method. ^^doc_ver. ^^doc_date.', action_name => '^^script..sql');
...
PRO
PRO Parameter 2:
PRO SQL_ID of the SQL to be analyzed (required) or press RETURN to get
PRO DB only mode (only non-SQL_ID report).
PRO
DEF input_sql_id = '^2';
PRO

variable sqlhc_mode varchar2(50);
variable db_script  varchar2(17);
column sqlhc_mode new_value sqlhc_mode
column db_script new_value db_script
--
set serveroutput on
declare
  sqlhc_mode varchar2(6):='SQL_ID';
  parameter_length number :=0;
  invalid_sql_id_length exception;
begin
  SELECT length('^^input_sql_id.') into parameter_length FROM dual;
  if parameter_length = 13 then
    :db_script := 'sqlhc_db_null.sql';
  elsif parameter_length is null then
    :db_script :='sqlhc_db.sql';
  else
    :db_script :='sqlhc_exit.sql';  
    raise invalid_sql_id_length;
  end if;
  exception
    when invalid_sql_id_length then
      dbms_output.put_line('Invalid SQL_ID length entered. Expected length 13 got length '||parameter_length);	
end;
/

select :db_script db_script from dual;
@@^^db_script. "^^input_license." 

DEF input_parameter = '^^input_sql_id.';
select nvl('^^input_sql_id.','DBMODE') input_sql_id from dual;

PRO Values passed:
PRO License: "^^input_license."
PRO Mode   : "SQL ID mode"
...
COL rdbms_version_full NEW_V rdbms_version_full FOR A17;
SELECT max(target_version) rdbms_version_full From sys.dba_registry_sqlpatch;

-- get platform
COL platform NEW_V platform FOR A80;
--SELECT UPPER(TRIM(REPLACE(REPLACE(product, 'TNS for '), ':' ))) platform 
--  FROM product_component_version WHERE product LIKE 'TNS for%' AND ROWNUM = 1;
select platform_name platform from v$database;
...
COL startup_time_stamp new_v startup_time_stamp for a20;
select max(to_char(startup_time,'YYYY-MM-DD/HH24:MI:SS')) startup_time_stamp from dba_hist_database_instance where rownum=1 order by startup_time desc ;
...
SELECT DISTINCT name, value
 ...
 WHERE :health_checks = 'Y'
   AND sql_id = :sql_id
   AND isdefault = 'NO' 
   AND name not like 'is_recur_flags'
   and name not like 'parallel%');
...
-- (006)
-- Major version of optimizer_features_enable <> Major version of rdbms_version at system level
INSERT INTO plan_table (id, operation, object_alias, other_tag, remarks, projection,position)
...
45
 ...
 WHERE :health_checks = 'Y'
   AND 
    SUBSTR    ('^^rdbms_version.', 1, instr('^^rdbms_version.','.')-1) 
    <> SUBSTR('^^sys_ofe.'       , 1, instr('^^sys_ofe.'      ,'.')-1);
...
SELECT DISTINCT value
 ...
 WHERE :health_checks = 'Y'
   ...
   AND SUBSTR('^^rdbms_version.', 1, instr('^^rdbms_version.','.')-1) <> substr(value,1,instr(value,'.')-1)) v; 
...
SELECT :E_GLOBAL, 'CBO PARAMETER', SYSTIMESTAMP, 'OPTIMIZER_DYNAMIC_SAMPLING',
       'Consider if you need to increase the value of OPTIMIZER_DYNAMIC_SAMPLING from ^^sys_ds. to a higher value as some objects have no statistics. See the MOS Note Optimizer Dynamic Statistics (OPTIMIZER_DYNAMIC_SAMPLING) (Doc ID 336267.1)',
...
SELECT :E_GLOBAL, 'DBMS_STATS', SYSTIMESTAMP, 'DBA_AUTOTASK_CLIENT',
 ...
 WHERE :health_checks = 'Y'
   ...
   and ('^^is_ebs.' = 'Y' or '^^is_siebel.' = 'Y' or '^^is_psft.' = 'Y')
   AND status = 'ENABLED';
...
COL files_prefix NEW_V files_prefix FOR A45;
SELECT '^^script._^^time_stamp._^^sql_id.' files_prefix FROM DUAL;
COL sqldx_prefix NEW_V sqldx_prefix FOR A45;
 ...
 *
 **************************************************************************************************/
-- Before we start the main report we must create the scripts needed 
-- later
-- sql_shared_cursor_sum.sql
-- sql_shared_cursor_col.sql
-- sql_shared_cursor_cur.sql
-- 
-- 
...
PRO        '</tr>'
PRO   FROM (
SELECT (CASE WHEN ROWNUM = 1 THEN 'WITH sc AS (SELECT /*+ MATERIALIZE */ /* 
  gv$sql_shared_cursor cursor_sum */ * FROM gv$sql_shared_cursor WHERE :shared_cursor = ''Y'' 
  AND sql_id = ''^^sql_id.'')' ELSE 'UNION ALL' END)||CHR(10)||
 ...
 *
 * ------------------------- */
SPO ^^files_prefix._1_main.html;

PRO <html>
PRO <!-- $Header: ^^mos_doc. ^^script..sql ^^doc_ver. ^^doc_date. $ -->
PRO <!-- Copyright (c) 2000-2012, Oracle Corporation. All rights reserved. -->
PRO
PRO <head>
PRO <title>^^files_prefix._1_main.html</title>
PRO
...
PRO a.l {font-weight:bold; color:#663300;} /* line in brownish */
PRO a.nl {text-decoration:none; color:#336699;} /* no line in blue */
PRO a.nlb {font-weight:normal; text-decoration:none; color:black;} /* no line in black */
PRO a.nlr {font-weight:normal; text-decoration:none; color:crimson;} /* no line in red */
PRO a.op {font-weight:normal; font:8pt Courier New; text-decoration:none; color:black;} /* no line in black and fixed font */
...
PRO h4 {font-size:10pt; font-weight:bold; color:#336699;}
...
PRO th.vt {writing-mode:tb-rl; filter:flipV flipH; text-align:center; padding-left:1pt; padding-right:1pt; padding-top:3pt; padding-bottom:3pt;}
...
PRO td.c {text-align:center; background:#fcfcf0;} /* center */
PRO td.l {text-align:left; background:#fcfcf0;} /* left (default) */
PRO td.r {text-align:right; background:#fcfcf0;} /* right */
...
PRO td.t {text-align:center; font-weight:bold; color:#336699; background:#cccc99;} /* center title */
PRO td.rt {text-align:right; font-weight:bold; color:#336699; background:#cccc99;} /* right title */
PRO td.lt {text-align:left; font-weight:bold; color:#336699; background:#cccc99;} /* left title */
PRO td.ltm {text-align:left; vertical-align:middle; font-weight:bold; color:#336699; background:#cccc99;} /* left title middle */
PRO td.w {background:white;} /* white */
PRO td.lw {text-align:left; background:white;} /* left and white */
PRO td.vt {background:#fcfcf0; writing-mode:tb-rl; filter:flipV flipH; text-align:center; padding-left:1pt; padding-right:1pt; padding-top:3pt; padding-bottom:3pt;}
PRO td.op {font:8pt Courier New; color:black; background:#fcfcf0;} /* black and fixed font */
PRO font.n {font-size:8pt; font-style:italic; color:#336699;} /* table footnote in blue */
PRO font.nr {font-size:8pt; font-style:italic; color:crimson;} /* table footnote in red */
PRO font.crimson {color:crimson;}
PRO font.red {color:red;}
PRO font.darkred {color:darkred;}
PRO font.orange {color:orange;}
PRO font.darkorange {color:darkorange;}
PRO font.green {color:green;}
PRO font.darkblue {color:#0000CC;}
PRO font.f {font-size:8pt; color:#999999;} /* footnote in gray */
PRO font.c {font:8pt Courier New;} /* fixed font */
PRO font.b {font-weight:bold;} /* bold */
PRO font.br {font-weight:bold; color:crimson;} /* bold and red */
PRO </style>
...
PRO ^^doc_ver. (^^doc_date.) Report: ^^files_prefix._1_main.html</h1>
PRO
PRO <a name="toc"></a> 
PRO <table border="0" ><tr><td class="w">
PRO <h3>Navigation</h3>
PRO <ul>
PRO <li><a href="file:^^files_prefix._3_execution_plans.html">Execution Plans</a>
PRO <li><a href="file:^^files_prefix._4_sql_detail.html">SQL Details Active Report</a>
PRO <li><a href="file:^^files_prefix._6_10053_trace_from_cursor.trc">10053 trace</a>
PRO <li><a href="file:^^files_prefix._10_sql_tuning_advisor.out">SQL Tuning Advisor Output</a>
PRO <li><a href="file:^^files_prefix._5_sql_monitor.zip">SQL Monitor Reports(s)</a>
PRO <li><a href="file:^^files_prefix._12_awr.zip">AWR reports</a>
PRO <li><a href="file:sqlhc_pxhcdr_^^database_name_short._^^rdbms_version._13_pxhcdr.zip">Parallel Execution Health Check (if Available)</a>
PRO <li><a href="file:^^files_prefix._2_startup_history.txt">Startup History</a>
PRO </li>
PRO </ul>
...
PRO <h3>Cursor Sharing and Binds</h3>
PRO <ul>
PRO <li><a href="#share_r">Cursor Sharing</a></li>
...
PRO </td><td class="w">&nbsp;&nbsp;&nbsp;&nbsp;</td><td class="w">

PRO <h3>Plan Control</h3>
PRO <ul>
PRO <li><a href="#spm">SQL Plan Baselines</a></li>
PRO <li><a href="#spm2">SQL Plan Baselines Config</a></li>
PRO <li><a href="#base">SQL Plan Baselines Evolution</a></li>
PRO <li><a href="#prof">SQL Profiles (DBA_SQL_PROFILES)</a></li>
PRO <li><a href="#patch">SQL Patches (DBA_SQL_PATCHES)</a></li>
PRO <li><a href="#STS">SQL Tuning Sets (DBA_SQLSET_STATEMENTS)</a.</li>
PRO </ul>
PRO <h3>Tables</h3>
PRO <ul>
PRO <li><a href="#tbl_sum">Tables Summary</a></li>
PRO <li><a href="#tables">Tables</a></li>
PRO <li><a href="#tbl_parts">Partitions</a></li>
PRO <li><a href="#idx_sum">Indexes</a></li>
PRO <li><a href="#tbl_cols">Columns</a></li>
PRO <li><a href="#tbl_histograms">Histograms</a></li>
PRO <li><a href="#tbl_constr">Constraints</a></li>
...
PRO </ul>	
PRO <h3>Diagnostics</h3>
PRO <ul>
PRO <li><a href="#adv">DBA Advisor Parameters</a></li>
PRO <li><a href="#ash_plan">Active Session History by Plan</a></li>
PRO <li><a href="#ash_line">Active Session History by Plan Line</a></li>
PRO <li><a href="#awr_plan">AWR Active Session History by Plan</a></li>
PRO <li><a href="#awr_line">AWR Active Session History by Plan Line</a></li>
...
PRO <li><a href="#non_def_hid">Hidden Parameters with Non-Default or Modified Values</a></li>
...
PRO </td></tr>
PRO </table>
PRO <font class="nr">This report may include some content provided by the Oracle Diagnostic and/or the Oracle Tuning Packs (in particular SQL Tuning Advisor "STA", SQL Tuning Sets "STS", SQL Monitoring and/or Automatic Workload Repository "AWR"). Be aware that using this extended functionality requires a license for the corresponding pack.</font>
PRO <br>
PRO <pre>
PRO License    : ^^input_license.
...
PRO Startup    : ^^startup_time_stamp.
...
SELECT CHR(10)||'<tr>'||CHR(10)||
       '<td class="rt">'||ROWNUM||'</td>'||CHR(10)||
       '<td class="l">'||v.object_type||'</td>'||CHR(10)||
       '<td class="l">'||v.priority||'</td>'||CHR(10)||
       '<td class="l">'||v.object_name||'</td>'||CHR(10)||
       '<td class="l">'||v.observation||'</td>'||CHR(10)||
       '<td class="l">'||v.more||'</td>'||CHR(10)||
       '</tr>'
  FROM (
SELECT /*+ NO_MERGE */
...
PRO </table>
PRO
PRO <font class="f"><a href="#toc">Go to Top</a></font>
...
PRO <a name="base"></a><h2>SQL Plan Baselines Evolution (DBA_SQL_PLAN_BASELINES)</h2>
PRO
PRO Available in 12c or higher.
PRO
PRO <table>
PRO <tr>
PRO <th>Origin</th>
...
SELECT
  ...
  '<td>'||count(*)||'</td>'||CHR(10)||
  '</tr>'
  FROM dba_sql_plan_baselines group by origin, accepted;
PRO <tr>
PRO <th>Origin</th>
PRO <th>Accepted</th>
PRO <th>Count</th>
...
PRO Collected from GV$SQL_SHARED_CURSOR.
PRO
REM There seems to be no SQL here for this heading???
REM
SELECT '<!-- '||TO_CHAR(SYSDATE, 'YYYY-MM-DD/HH24:MI:SS')||' -->' FROM dual;
PRO <!-- Please Wait -->
PRO <br><font class="f"><a href="#toc">Go to Top</a></font>
...
/* ----------------------------------------------------------------------------
 *
 * SQL Tuning Sets Statements
 *
 * --------------------------------------------------------------------------*/
PRO <a name="STS"></a><h2>SQL Tuning Set Information (DBA_SQLSET_STATEMENTS)</h2>
PRO
PRO Available in 12c or higher.
PRO
PRO <table>
PRO <tr>
PRO <th>SQL Set Name</th>
PRO <th>N Plans</th>
PRO <th>Distinct Statements</th>
PRO </tr>
PRO
SELECT '<!-- '||TO_CHAR(SYSDATE, 'YYYY-MM-DD/HH24:MI:SS')||' -->' FROM dual;
PRO <!-- Please Wait -->
SELECT
  ...
  '<td>'||count(*)||'</td>'||CHR(10)||
  '<td>'||count(distinct sql_id)||'</td>'||CHR(10)||
  '</tr>'
  FROM dba_sqlset_statements s, dba_users u where s.parsing_schema_name = u.username
  and oracle_maintained = 'N' group by sqlset_name order by sqlset_name;
PRO <tr>
PRO <th>SQL Set Name</th>
PRO <th>N Plans</th>
PRO <th>Distinct Statements</th>
...
EXEC DBMS_APPLICATION_INFO.SET_CLIENT_INFO('^^method.: Table Columns histograms - ' || TO_CHAR(SYSDATE, 'YYYY-MM-DD/HH24:MI:SS'));
PRO <a name="tbl_histograms"></a><h2>Table Column Histograms</a></h2>
PRO
SELECT '<!-- '||TO_CHAR(SYSDATE, 'YYYY-MM-DD/HH24:MI:SS')||' -->' FROM dual;
PRO <!-- Please Wait -->
...
set serveroutput on
declare 
  type histogram_row is record
  (
    endpoint_number number,
    endpoint_value number,
    endpoint_actual_value varchar2(255),
    estimated_endpoint_value varchar2(255),
    estimated_cardinality number, 
    estimated_selectivity number
  );
  type histogram_table is table of histogram_row; 
  histogram histogram_table;


  type schema_table_row is record (table_owner varchar2(30), table_name varchar2(255));
  type schema_table_table is table of schema_table_row;
  list_of_schema_tables schema_table_table;

  type column_row is record (column_name varchar2(255));
  type column_table is table of column_row;
  list_of_columns column_table;
  ...
  p_data_type varchar2(128); -- As seen in dba_tab_cols;
  histogram_type varchar2(15);
...
--
  ...
  BEGIN
    l_ownname := clean_object_name(p_ownname);
    l_tabname := clean_object_name(p_tabname);
    l_colname := clean_object_name(p_colname);
    l_partname := clean_object_name(p_partname);
    ...
    EXCEPTION
      WHEN NO_DATA_FOUND THEN
        dbms_output.put_line(p_ownname||'.'||p_tabname||'.'||p_colname||' not found');
        RETURN NULL;
    END;
    ...
    IF p_partname IS NULL THEN
      ...
      EXCEPTION
        WHEN NO_DATA_FOUND THEN
          dbms_output.put_line(p_ownname||'.'||p_tabname||'.'||NVL(p_partname, '<partname>')||'.'||p_colname||' not found');
          RETURN NULL;
      END;
    ...
    BEGIN
      ...
      ELSE
        dbms_output.put_line(p_ownname||'.'||p_tabname||'.'||NVL(p_partname, '<partname>')||'.'||p_colname||' has no statistics');
      END IF;
    EXCEPTION
      WHEN OTHERS THEN
        dbms_output.put_line(SQLERRM);
        dbms_output.put_line('While executing SYS.DBMS_STATS.GET_COLUMN_STATS on '||p_ownname||'.'||p_tabname||'.'||NVL(p_partname, '<partname>')||'.'||p_colname);
    END;
...
begin
--
-- We need to loop on every schema
-- Every Table
-- Every column
--
  select data_type into p_data_type from sys.dba_tab_cols where owner='STELIOS' and table_name='EMPLOYEES' and column_name='DEPARTMENT_ID';
  select object_owner, object_name bulk collect into list_of_schema_tables from plan_table where object_type='TABLE';
  dbms_output.put_line('<ul>');
--
-- List the tables
--
  for m in 1..list_of_schema_tables.count
  loop
    dbms_output.put_line('<a href="#hist_'||list_of_schema_tables(m).table_name||'">'||list_of_schema_tables(m).table_name||'</a><BR>');
  end loop;
  dbms_output.put_line('</ul>');
  for i in 1..list_of_schema_tables.count
  loop
    dbms_output.put_line('<a name="hist_'||list_of_schema_tables(i).table_name||'"></a>');
    dbms_output.put_line('<h3>'||list_of_schema_tables(i).table_owner||'.'||list_of_schema_tables(i).table_name||'</h3>'||CHR(10)||CHR(10));
--
-- For every schema.table get the columns
--
    select column_name bulk collect into list_of_columns from dba_tab_columns 
      where owner=list_of_schema_tables(i).table_owner
      and table_name=list_of_schema_tables(i).table_name;
    for j in 1..list_of_columns.count
    loop
      l_prior_endpoint_number := 0;
--
-- Before we write a Histogram for a column make sure it has a histogram
--
      select histogram into histogram_type from dba_tab_columns d
        where d.owner=list_of_schema_tables(i).table_owner
        and d.table_name=list_of_schema_tables(i).table_name
        and d.column_name=list_of_columns(j).column_name;
      if histogram_type != 'NONE' then
--
-- Write the SCHEMA.TABLE header
--
        dbms_output.put_line('<h4>'||list_of_schema_tables(i).table_owner||'.'||list_of_schema_tables(i).table_name||'.'||list_of_columns(j).column_name||'</h4>'||CHR(10)||CHR(10));
--
-- The values for estimated Cardinality and Estimated Selectivity are calculated in sqcpkgt.pkb line 980
--
        select num_rows into l_num_rows from dba_tables where table_name=list_of_schema_tables(i).table_name
	  and owner=list_of_schema_tables(i).table_owner;
	select num_nulls into l_num_nulls from dba_tab_columns where table_name=list_of_schema_tables(i).table_name
	  and owner=list_of_schema_tables(i).table_owner 
	  and column_name= list_of_columns(j).column_name;

        l_adjust_4_nulls := (l_num_rows - l_num_nulls)/greatest(1, l_num_rows);
        select 
          endpoint_number, 
          endpoint_value,   
          ENDPOINT_ACTUAL_VALUE,
          to_char(endpoint_actual_value), -- For DATE datatypes this is not true
	  0,
          0
        bulk collect into histogram 
        from dba_tab_histograms where owner=list_of_schema_tables(i).table_owner 
          and table_name=list_of_schema_tables(i).table_name
          and column_name=list_of_columns(j).column_name;
        l_bucket_count := histogram.count;
        dbms_output.put_line(histogram_type||' histogram with '||l_bucket_count||' buckets. Number of rows in this table is '||l_num_rows||'.'||'Number of nulls in this column is '||l_num_nulls);
--
-- Now we need to figure out how to get Estimated Cardinality and Estimated Selectivity
--
-- Get the column data type
-- DATE:TO_DATE(TO_CHAR(TRUNC(p_endpoint_value)), 'J') + (p_endpoint_value - TRUNC(p_endpoint_value));l_return := TO_CHAR(l_date, 'S'||DATE_FORMAT);
-- NUMBER:  TO_CHAR(p_endpoint_value);
-- Then use get_external_value from sqcpkgs.pkb line 413 
--      select object_name into p_object_name from plan_table where object_type='TABLE' group by object_name; 
--
        dbms_output.put_line('<table>');
        dbms_output.put_line('<tr>');
        dbms_output.put_line('<th>#</th>');
        dbms_output.put_line('<th>Endpoint Number</th>');
        dbms_output.put_line('<th>Endpoint Value</th>');
        dbms_output.put_line('<th>Endpoint Actual Value</th>');
        dbms_output.put_line('<th>Estimated Endpoint Value</th>');
        dbms_output.put_line('<th>Estimated Cardinality</th>');
        dbms_output.put_line('<th>Estimated Selectivity</th>');
        dbms_output.put_line('</tr>');
        dbms_output.put_line('');
        for k in histogram.first..histogram.last 
        loop
          l_gap_endpoint_number := histogram(k).endpoint_number - l_prior_endpoint_number;
          l_estimated_cardinality:=CEIL(l_adjust_4_nulls * l_gap_endpoint_number);
          l_estimated_selectivity:=lpad(to_char(round(l_estimated_cardinality / (l_num_rows - l_num_nulls),6)),8,'0');
          dbms_output.put_line(
            CHR(10)||'<tr>'||CHR(10)||
            '<td class="rt">'||k||'</td>'||CHR(10)||
            '<td class="l">'||histogram(k).endpoint_number         ||'</td>'||CHR(10)||
            '<td class="l">'||histogram(k).endpoint_value          ||'</td>'||CHR(10)||
            '<td class="l">'||histogram(k).endpoint_actual_value   ||'</td>'||CHR(10)||
            '<td class="l">'||histogram(k).estimated_endpoint_value||'</td>'||CHR(10)||
            '<td class="l">'||l_estimated_cardinality              ||'</td>'||CHR(10)||
            '<td class="l">'||l_estimated_selectivity              ||'</td>'||CHR(10)||
            '</tr>'
          );
          l_prior_endpoint_number := histogram(k).endpoint_number;
        end loop;
        dbms_output.put_line('<tr>');
        dbms_output.put_line('<th>#</th>');
        dbms_output.put_line('<th>Endpoint Number</th>');
        ...
        dbms_output.put_line(''); 
        dbms_output.put_line('</table>');
        dbms_output.put_line('<font class="f"><a href="#tbl_histograms">Go to Column Histograms</a></font><BR>');
        dbms_output.put_line('<font class="f"><a href="#toc">Go to Top</a></font>');
      end if;
    end loop;
  end loop;
end;
/
--
-- sqcpkgm.pkb 12802-12815
--
-- SQLT$_dba_tab_histograms
-- dba_tab_histograms
-- 
PRO
ROLLBACK TO save_point_1;
...
SELECT object_name table_name,
       ...
       4 line_type,
       1 row_num,
       CHR(10)||'</table>'||CHR(10)||CHR(10)||'<font class="n">"C": Check constraint. "P": Primary key. "U": Unique constraint. "R": Reference constraint</font><br>' line_text
  FROM plan_tables
 WHERE object_type = 'TABLE') v2
 ORDER BY
       v2.table_name,
...
-- Views
--
--column view_text format a254 word_wrapped
--PRO <pre>
WITH object AS (
SELECT /*+ MATERIALIZE */
...
SELECT object_owner owner, object_name view_name, object_type, 'DBA_HIST_SQL_PLAN' source
  FROM dba_hist_sql_plan
 WHERE :license IN ('T', 'D') 
--and timestamp > sysdate - interval 'days.' day))
   AND dbid = ^^dbid.
   AND sql_id = :sql_id
   AND object_owner IS NOT NULL
   AND object_name IS NOT NULL
...
SELECT to_owner owner, to_name view_name, decode(to_type,2,'MAT_VIEW',4,'VIEW'), 'OBJ_DEPENDENCY' source
 ...
 WHERE to_owner <> 'SYS'
   and to_type IN (4) -- 2.tables/materialized view table, 4=views
...
views AS (
  SELECT /*+ MATERIALIZE */ distinct owner, view_name, object_type FROM object
 UNION
...
dviews AS (
 SELECT /*+ MATERIALIZE */ DISTINCT owner, view_name, Decode(object_type,'MATERIALIZED VIEW','MAT_VIEW',object_type) object_type FROM views o
  WHERE (exists (select 1 from dba_views v where v.view_name=o.view_name and v.owner = o.owner and v.editioning_view ='N')  -- eliminate editioning views
             or Decode(object_type,'MATERIALIZED VIEW','MAT_VIEW',object_type)='MAT_VIEW')                                   -- but not MVs
)
SELECT '<h3><br><summary id="summary1">View: '|| s.owner||'.'||s.view_name||'</h3>'||REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(DBMS_METADATA.GET_DDL('VIEW',s.view_name,s.owner), '<', CHR(38)||'lt;'), '>', CHR(38)||'gt;'), '''', CHR(38)||'#39;'), '"', CHR(38)||'quot;'), CHR(10), '<br>'||CHR(10))
       as view_text
  FROM dviews s
 ORDER BY
       s.owner,
...
SET HEA ON PAGES 50 MARK HTML ON TABLE "" SPOOL OFF;
COL signature FOR 99999999999999999999;
SELECT ROWNUM "#", v.* FROM (SELECT /*+ NO_MERGE */ signature, sql_handle,
  substr(sql_text,0,60) SQL_TEXT, plan_name, creator, origin, parsing_schema_name, description,
  version, created, last_modified, last_executed, last_verified,
  enabled, accepted, fixed, reproduced, autopurge, adaptive,
  optimizer_cost, module, action, executions, elapsed_time,
  cpu_time, buffer_gets, disk_reads, direct_writes, rows_processed,
  fetches, end_of_fetch_count FROM
  dba_sql_plan_baselines WHERE signature IN (^^signature., ^^signaturef.) ORDER BY created, plan_name) v;
PRO
PRO
SET HEA OFF PAGES 0 MARK HTML OFF;
PRO <font class="f"><a href="#toc">Go to Top</a></font>
/*--------------------------
 *
 * dba_sql_plan_management_config
 *
 * ------------------------- */

PRO <a name="spm2"></a><h2>SQL Plan Baselines Config (DBA_SQL_PLAN_MANAGEMENT_CONFIG)</h2>
PRO
PRO Available in 12c or higher.
PRO
PRO <table>
...
SELECT
  CHR(10)||'<tr>'||CHR(10)||
  '<td class="r">'||ROWNUM||'</td>'||CHR(10)||
  '<td>'||parameter_name||'</td>'||CHR(10)||
  '<td>'||parameter_value||'</td>'||CHR(10)||
  '<td>'||last_modified||'</td>'||CHR(10)||
  '<td>'||modified_by||'</td>'||CHR(10)||
  '</tr>'
  FROM dba_sql_management_config  ORDER BY parameter_name;
PRO <tr>
PRO <th>#</th>
PRO <th>Parameter Name</th>
PRO <th>Parameter Value</th>
...
PRO <a name="adv"></a><h2>DBA Advisor Parameters (DBA_ADVISOR_PARAMETERS)</h2>
PRO
PRO Available on 10g or higher.
PRO
PRO <table>
PRO <tr>
PRO <th>#</th>
...
PRO <th>PARAMETER_TYPE</th>
PRO <th>IS_DEFAULT</th>
PRO <th>IS_OUTPUT</th>
PRO <th>IS_MODIFIABLE_ANYTIME</th>
PRO <th>DESCRIPTION</th>
PRO <th>EXECUTION_TYPE</th>
PRO </tr>
PRO
SELECT '<!-- '||TO_CHAR(SYSDATE, 'YYYY-MM-DD/HH24:MI:SS')||' -->' FROM dual;
PRO <!-- Please Wait -->
SELECT
  ...
  '<td>'||parameter_name||'</td>'||CHR(10)||
  '<td>'||parameter_value||'</td>'||CHR(10)||
  '<td>'||parameter_type||'</td>'||CHR(10)||
  '<td>'||is_default||'</td>'||CHR(10)||
  '<td>'||is_output||'</td>'||CHR(10)||
  '<td>'||is_modifiable_anytime||'</td>'||CHR(10)||
  '<td>'||description||'</td>'||CHR(10)||
  '<td>'||execution_type||'</td>'||CHR(10)||
  '</tr>'
  FROM dba_advisor_parameters where owner='SYS' and task_name='SYS_AUTO_SPM_EVOLVE_TASK' and parameter_value !='UNUSED' order by rownum, parameter_name;
PRO <tr>
PRO <th>#</th>
PRO <th>OWNER</th>
PRO <th>TASK ID</th>
...
PRO <!-- Please Wait -->
SELECT
  case when substr('^^rdbms_version.',1,instr('^^rdbms_version.','.')-1) > '11'
  then
    CHR(10)||'<tr>'||CHR(10)||
    '<td class="r">'||ROWNUM||'</td>'||CHR(10)||
    ...
    '<td class="r">'||full_plan_hash_value||'</td>'||CHR(10)||
...
PRO <th>Full Plan<br>Hash<br>Value</th>
...
SELECT CHR(10)||'<tr>'||CHR(10)||
       ...
       '<td class="r">'||v.sql_full_plan_hash_value||'</td>'||CHR(10)||
...
/* -------------------------
 *
 * Hidden parameters with non-Default Values
 *
 * ------------------------- */
EXEC DBMS_APPLICATION_INFO.SET_CLIENT_INFO('^^method.: Hidden Non-Default Parameters - ' || TO_CHAR(SYSDATE, 'YYYY-MM-DD/HH24:MI:SS'));
PRO <a name="non_def_hid"></a><h2>Hidden Non-Default Parameters Values</h2>
PRO
PRO Collected from GV$SYSTEM_PARAMETER2 where isdefault = 'FALSE' OR ismodified != 'FALSE' and name starts with underscore.
...
SELECT /* ^^script..sql Hidden non-default Parameters */
        CHR(10)||'<tr>'||CHR(10)||
        '<td class="r">'||ROWNUM||'</td>'||CHR(10)||
        '<td>'||v.name||'</td>'||CHR(10)||
 ...
 SELECT /*+ NO_MERGE */ *
  ...
  WHERE (isdefault = 'FALSE' OR ismodified <> 'FALSE') and value is not null
...
SELECT /*+ NO_MERGE */ *
  FROM gv$system_parameter2
 WHERE (isdefault = 'FALSE' OR ismodified <> 'FALSE') and name not like '\_%' escape '\'
 ORDER BY
       name,
       inst_id,
       ordinal ) v;
...
SPO OFF;
EXEC DBMS_APPLICATION_INFO.SET_CLIENT_INFO('^^method.: Startup History - ' || TO_CHAR(SYSDATE, 'YYYY-MM-DD/HH24:MI:SS'));
SPO ^^files_prefix._2_startup_history.txt;
select to_char(startup_time,'DD-MON-YYYY HH24:MI:SS') "Startup Time" from  dba_hist_database_instance order by startup_time desc;
SPO OFF;
...
PRO <li><a href="file:^^files_prefix._1_main.html">Go Back To Main Report</a></li>
PRO </ul>
...
FROM 
  gv$sql v,
  TABLE(DBMS_XPLAN.DISPLAY('gv$sql_plan_statistics_all', NULL, 'ADVANCED ALLSTATS LAST -QBREGISTRY', 'inst_id = '||v.inst_id||' AND sql_id = '''||v.sql_id||''' AND child_number = '||v.child_number)) t
 WHERE 
   v.sql_id = :sql_id
...
SELECT RPAD('Inst: '||v.inst_id, 9)||' '||RPAD('Child: '||v.child_number, 11) inst_child, trim(t.plan_table_output)
  FROM gv$sql v,
       TABLE(DBMS_XPLAN.DISPLAY('gv$sql_plan_statistics_all', NULL, 'ADVANCED ALLSTATS -QBREGISTRY', 'inst_id = '||v.inst_id||' AND sql_id = '''||v.sql_id||''' AND child_number = '||v.child_number)) t
...
SELECT t.plan_table_output
  FROM (SELECT DISTINCT sql_id, plan_hash_value, dbid
       ...
       TABLE(DBMS_XPLAN.DISPLAY_AWR(v.sql_id, v.plan_hash_value, v.dbid, 'ADVANCED -QBREGISTRY')) t;
...
PRO end -->
SELECT '<!-- '||TO_CHAR(SYSDATE, 'YYYY-MM-DD/HH24:MI:SS')||' -->' FROM dual;
SELECT :det||'<html><ul><li><a href="file:^^files_prefix._1_main.html">Click Back Arrow on Browser to Return to the Main Report</a></li></ul></html>' FROM DUAL;
SELECT '<!-- '||TO_CHAR(SYSDATE, 'YYYY-MM-DD/HH24:MI:SS')||' -->' FROM dual;
SPO OFF;
...
HOS zip -m ^^files_prefix..zip ^^files_prefix._1_main.html
...
HOS zip -m ^^files_prefix._7_driver.zip sql_shared_cursor_sum_^^sql_id..sql
HOS zip -m ^^files_prefix._7_driver.zip sql_shared_cursor_col_^^sql_id..sql
HOS zip -m ^^files_prefix._7_driver.zip sql_shared_cursor_cur_^^sql_id..sql
HOS zip -m ^^files_prefix._9_log.zip ^^files_prefix._tkprof_*.txt
HOS zip -m ^^files_prefix._7_driver.zip ^^files_prefix._5_sql_monitor.sql
...
select case(select count(*) is_windows from v$database where lower(platform_name) like '%windows%')
  when 0 then 
    'if [ -e ^^adr_home_trace./trace/*_DBMS_SQLDIAG_10053_^^unique_id.*.trc ] ; then cp ^^adr_home_trace./trace/*_DBMS_SQLDIAG_10053_^^unique_id.*.trc ./^^files_prefix._6_10053_trace_from_cursor.trc ; else echo No 10053 trace generated> ./^^files_prefix._6_10053_trace_from_cursor.trc; fi'
  else 'if exist ^^adr_home_trace.\trace\*_DBMS_SQLDIAG_10053_^^unique_id.*.trc (copy/y ^^adr_home_trace.\trace\*_DBMS_SQLDIAG_10053_^^unique_id.*.trc ^^files_prefix._6_10053_trace_from_cursor.trc) else (echo No 10053 trace generated>./^^files_prefix._6_10053_trace_from_cursor.trc)'
  end host_cmd
  from dual;
...
select case ( select count(*) from v$pdbs) 
  ...
  else 'touch ./oratcb_^^sql_id._NO_TCB_ON_PDB' end cp_source_files from dba_directories where directory_name='DATA_PUMP_DIR';
...
--
-- Check if the SQL_ID in question has ANY plans with a PX element by querying dbms_xplan.display.
--

variable px_script varchar2(50);
column px_script new_value px_script
--
set serveroutput on
declare
  parallel_lines_mem number:=0;
  parallel_lines_awr number:=0;
  is_windows number:=0;
begin
  select count(*) into is_windows from v$database where lower(platform_name) like '%windows%';
  SELECT count(*) into parallel_lines_mem FROM gv$sql_plan_statistics_all WHERE sql_id = '^^sql_id.' and operation like 'PX%';
  SELECT count(t.plan_table_output) into parallel_lines_awr 
    FROM (SELECT DISTINCT sql_id, plan_hash_value, dbid FROM dba_hist_sql_plan WHERE sql_id = '^^sql_id.') v, 
    TABLE(DBMS_XPLAN.DISPLAY_AWR(v.sql_id, v.plan_hash_value, v.dbid, 'ADVANCED -QBREGISTRY')) t where t.plan_table_output like '|%|%PX%';
  dbms_output.put_line('parallel_lines_mem: '||parallel_lines_mem);
  dbms_output.put_line('parallel_lines_awr: '||parallel_lines_awr);
  dbms_output.put_line('is_windows: '||is_windows);
  if (parallel_lines_mem + parallel_lines_awr )>0 and is_windows = 0 then
    dbms_output.put_line('At least one parallel plan found for this SQL ID');
    :px_script :='sqlhc_pxhcdr.sql';
  else
    dbms_output.put_line('No PX lines or we are on Windows');
    :px_script :='sqlhc_pxhcdr_null.sql';
  end if;
end;
/

select :px_script px_script from dual;
@@^^px_script "^^input_license." "^^files_prefix" "^^files_prefix."
--
-- Collect 5 AWR reports
-- 

set linesize 500 heading off feedback off timing off headsep off echo off recsep off newpage none sqlprompt ''

spool ^^files_prefix._awr_1.sql
select * from (select * from (select * from (SELECT  'select output from table(sys.dbms_workload_repository.awr_report_html(l_dbid=>'||e.dbid|| ',l_inst_num=>'||e.instance_number|| ',l_bid=>'||b.snap_id|| ',l_eid=>'||e.snap_id|| ',l_options=>8));' FROM dba_hist_snapshot b, dba_hist_sqlstat e WHERE b.snap_id = e.snap_id - 1 AND b.dbid = e.dbid AND b.instance_number = e.instance_number and b.begin_interval_time > sysdate -3 GROUP BY b.begin_interval_time, e.dbid, e.instance_number, b.snap_id, e.snap_id ORDER BY sum(e.elapsed_time_delta) DESC) where rownum <6 offset 0 rows) where rownum=1);
spool off
spool ^^files_prefix._awr_1.html
@^^files_prefix._awr_1.sql
spool off
host zip -m ^^files_prefix._12_awr.zip ./^^files_prefix._awr_1.html
host zip -m ^^files_prefix._7_driver.zip ./^^files_prefix._awr_1.sql

spool ^^files_prefix._awr_2.sql
select * from (select * from (select * from (SELECT  'select output from table(sys.dbms_workload_repository.awr_report_html(l_dbid=>'||e.dbid|| ',l_inst_num=>'||e.instance_number|| ',l_bid=>'||b.snap_id|| ',l_eid=>'||e.snap_id|| ',l_options=>8));' FROM dba_hist_snapshot b, dba_hist_sqlstat e WHERE b.snap_id = e.snap_id - 1 AND b.dbid = e.dbid AND b.instance_number = e.instance_number and b.begin_interval_time > sysdate -3 GROUP BY b.begin_interval_time, e.dbid, e.instance_number, b.snap_id, e.snap_id ORDER BY sum(e.elapsed_time_delta) DESC) where rownum <6 offset 1 rows) where rownum=1);
spool off
spool ^^files_prefix._awr_2.html
@^^files_prefix._awr_2.sql
spool off
host zip -m ^^files_prefix._12_awr.zip ./^^files_prefix._awr_2.html
host zip -m ^^files_prefix._7_driver.zip ./^^files_prefix._awr_2.sql

spool ^^files_prefix._awr_3.sql
select * from (select * from (select * from (SELECT  'select output from table(sys.dbms_workload_repository.awr_report_html(l_dbid=>'||e.dbid|| ',l_inst_num=>'||e.instance_number|| ',l_bid=>'||b.snap_id|| ',l_eid=>'||e.snap_id|| ',l_options=>8));' FROM dba_hist_snapshot b, dba_hist_sqlstat e WHERE b.snap_id = e.snap_id - 1 AND b.dbid = e.dbid AND b.instance_number = e.instance_number and b.begin_interval_time > sysdate -3 GROUP BY b.begin_interval_time, e.dbid, e.instance_number, b.snap_id, e.snap_id ORDER BY sum(e.elapsed_time_delta) DESC) where rownum <6 offset 2 rows) where rownum=1);
spool off
spool ^^files_prefix._awr_3.html
@^^files_prefix._awr_3.sql
spool off
host zip -m ^^files_prefix._12_awr.zip ./^^files_prefix._awr_3.html
host zip -m ^^files_prefix._7_driver.zip ./^^files_prefix._awr_3.sql

spool ^^files_prefix._awr_4.sql
select * from (select * from (select * from (SELECT  'select output from table(sys.dbms_workload_repository.awr_report_html(l_dbid=>'||e.dbid|| ',l_inst_num=>'||e.instance_number|| ',l_bid=>'||b.snap_id|| ',l_eid=>'||e.snap_id|| ',l_options=>8));' FROM dba_hist_snapshot b, dba_hist_sqlstat e WHERE b.snap_id = e.snap_id - 1 AND b.dbid = e.dbid AND b.instance_number = e.instance_number and b.begin_interval_time > sysdate -3 GROUP BY b.begin_interval_time, e.dbid, e.instance_number, b.snap_id, e.snap_id ORDER BY sum(e.elapsed_time_delta) DESC) where rownum <6 offset 3 rows) where rownum=1);
spool off
spool ^^files_prefix._awr_4.html
@^^files_prefix._awr_4.sql
spool off
host zip -m ^^files_prefix._12_awr.zip ./^^files_prefix._awr_4.html
host zip -m ^^files_prefix._7_driver.zip ./^^files_prefix._awr_4.sql

spool ^^files_prefix._awr_5.sql
select * from (select * from (select * from (SELECT  'select output from table(sys.dbms_workload_repository.awr_report_html(l_dbid=>'||e.dbid|| ',l_inst_num=>'||e.instance_number|| ',l_bid=>'||b.snap_id|| ',l_eid=>'||e.snap_id|| ',l_options=>8));' FROM dba_hist_snapshot b, dba_hist_sqlstat e WHERE b.snap_id = e.snap_id - 1 AND b.dbid = e.dbid AND b.instance_number = e.instance_number and b.begin_interval_time > sysdate -3 GROUP BY b.begin_interval_time, e.dbid, e.instance_number, b.snap_id, e.snap_id ORDER BY sum(e.elapsed_time_delta) DESC) where rownum <6 offset 4 rows) where rownum=1);
spool off
spool ^^files_prefix._awr_5.html
@^^files_prefix._awr_5.sql
spool off
host zip -m ^^files_prefix._12_awr.zip ./^^files_prefix._awr_5.html
host zip -m ^^files_prefix._7_driver.zip ./^^files_prefix._awr_5.sql
host zip -m ^^files_prefix..zip ^^files_prefix._12_awr.zip
host zip -m ^^files_prefix..zip ^^files_prefix._7_driver.zip 
host zip -m ^^files_prefix..zip ^^files_prefix._2_startup_history.txt
...
CL COL;
UNDEF 1 2 method script mos_doc doc_ver doc_date doc_link bug_link input_parameter input_sql_id input_license unique_id sql_id signature signaturef license udump_path adr_home_trace sqldx_prefix sqldx_output db_script;
