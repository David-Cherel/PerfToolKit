REM
REM $Header: 1460440.1 pxhcdr.sql 12.1.09 2013/06/13 $
REM
...
REM
REM   Since sqlhc_pxhcdr.sql is a read-only script, which installs nothing
...
REM   SQL> START [path]sqlhc_pxhcdr.sql [T|D|N] [sql_id]
REM
REM NOTES
REM   1. For possible errors see sqlhc_pxhcdr.log.
...
REM    scharala    12/24/23 - Add pxhcdr report when PX query
Rem    scharala    12/24/23 - Created
Rem
@@?/rdbms/admin/sqlsessstart.sql
SPO sqlhc_pxhcdr.log
SET DEF '^'
set TERM OFF 
set ECHO ON VER OFF 
...
REM PRO
REM PRO Parameter 1:
REM PRO Oracle Pack License (Tuning, Diagnostics or None) [T|D|N] (required)
REM PRO
DEF input_license = '^1';
REM PRO
REM PRO Parameter 2:
REM PRO Target directory path for script output (optional)
REM PRO
REM DEF output_path = '^2';
REM PRO
SET TERM OFF;
...
REM COL full_path NEW_V full_path;
REM SELECT TRIM('^^output_path.')||CASE
REM WHEN INSTR('^^output_path.', '/') > 0 THEN '/'
REM WHEN INSTR('^^output_path.', '\') > 0 THEN '\'
REM END full_path
REM FROM DUAL;
...
REM BEGIN
REM   IF '^^license.' IS NULL OR '^^license.' NOT IN ('T', 'D', 'N') THEN
REM    RAISE_APPLICATION_ERROR(-20100, 'Oracle Pack License (Tuning, Diagnostics or None) must be specified as "T" or "D" or "N".');
REM   END IF;
REM END;
REM /
...
DEF script = 'sqlhc_pxhcdr';
DEF method = 'SQLHC_PXHCDR';
...
REM -- tracing script in case it takes long to execute so we can diagnose it
REM ALTER SESSION SET TRACEFILE_IDENTIFIER = "^^script._^^unique_id.";
 ...
 *
 * ------------------------- */
SPO ^^script._^^database_name_short._^^host_name_short._^^rdbms_version._main.html;
...
PRO
PRO <head>
PRO <title>^^script._^^database_name_short._^^host_name_short._^^rdbms_version._main.html</title>
PRO
...
PRO ^^doc_ver. Report: ^^script._^^database_name_short._^^host_name_short._^^rdbms_version._main.html</h1>
PRO
 ...
 *
 * ------------------------- */
SPO ^^script._^^database_name_short._^^host_name_short._^^rdbms_version._dop.html;
...
PRO <body>
PRO <h1><a target="MOS" href="^^doc_link.^^mos_doc.">^^mos_doc.</a> ^^method.
PRO ^^doc_ver. Report: ^^script._^^database_name_short._^^host_name_short._^^rdbms_version._dop.html</h1>
PRO
 ...
 *
 * ------------------------- */
SPO ^^script._^^database_name_short._^^host_name_short._^^rdbms_version._rscr.html;
...
PRO
PRO <head>
PRO <title>^^script._^^database_name_short._^^host_name_short._^^rdbms_version._rscr.html</title>
PRO
...
PRO <body>
PRO <h1><a target="MOS" href="^^doc_link.^^mos_doc.">^^mos_doc.</a> ^^method.
PRO ^^doc_ver. Report: ^^script._^^database_name_short._^^host_name_short._^^rdbms_version._rscr.html</h1>
PRO
...
SPO ^^script._^^database_name_short._^^host_name_short._^^rdbms_version._monitor.sql;
...
--      '   AND px_servers_requested > 1 '||
    ...
    LOOP
      ...
      DBMS_OUTPUT.PUT_LINE('SPO ^^script._^^database_name_short._^^host_name_short._^^rdbms_version._'||mon_rec.sql_id||'_monitor.html;');
...
-- 11g
@^^script._^^database_name_short._^^host_name_short._^^rdbms_version._monitor.sql
...
-- log zip
HOS zip -mT ^^script._^^database_name_short._^^rdbms_version._log.zip sql_shared_cursor.sql
HOS zip -mT ^^script._^^database_name_short._^^rdbms_version._log.zip ^^script..log
-- 11g

-- 11g monitor zip
HOS zip -mT ^^script._^^database_name_short._^^rdbms_version._monitor.zip ^^script._^^database_name_short._^^host_name_short._^^rdbms_version._*_monitor.*

-- main zip
HOS zip -mT ^^script._^^database_name_short._^^rdbms_version._13_pxhcdr.zip ^^script._^^database_name_short._^^host_name_short._^^rdbms_version._*
HOS zip -mT ^^script._^^database_name_short._^^rdbms_version._13_pxhcdr.zip ^^script._^^database_name_short._^^rdbms_version._log.zip
HOS zip -mT ^^script._^^database_name_short._^^rdbms_version._13_pxhcdr.zip ^^script._^^database_name_short._^^rdbms_version._monitor.zip
HOS zip -m '^^3' ^^script._^^database_name_short._^^rdbms_version._13_pxhcdr.zip
...
PRO ^^script._^^database_name_short._^^host_name_short._^^rdbms_version._13_pxhcdr.zip.
PRO
CL COL;
REM SET DEF ON;
