...
-- #############################################################################################################
--
spool table_stats_complete.log
...
exec  dbms_output.put_line('  Table Statistic Differences : comparison from now against yesterday');
exec  dbms_output.put_line('==========================================================================================');

select report from table(dbms_stats.diff_table_stats_in_history(upper('&&ownname'),upper('&&tabname'),systimestamp-1));
...
begin
  ...
  v_query := 'select owner,table_name, last_analyzed, STATTYPE_LOCKED STATS_LOCKED, 
								 STALE_STATS,GLOBAL_STATS,USER_STATS, 
                                 num_rows rows_in_table, chain_cnt, blocks, empty_blocks, avg_space,
  ...
  if v_ct > 0 then
          ...
          v_query := 'select partition_name, last_analyzed, high_value,
                                         num_rows rows_in_partition, chain_cnt, blocks, empty_blocks,
...
*/
  ...
  if v_ct > 0 then
          ...
          v_query := 'select a.index_name, substr(a.index_type, 1, 4) index_type,a.UNIQUENESS,
