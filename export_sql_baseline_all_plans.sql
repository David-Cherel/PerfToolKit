...
-- #############################################################################################################
--
spool export_sql_baseline_all_plans.log
...
dbms_output.put_line('Creation of staging table for loading ALL SQL Plans from Baseline, table name : '||l_table_name);
dbms_spm.create_stgtab_baseline(table_name => l_table_name,table_owner => l_table_owner,tablespace_name => l_tablespace);

dbms_output.put_line('Loading ALL SQL Plan from SQL Handle : '||l_sql_handle||' into staging table : '||l_table_name);
ret := dbms_spm.pack_stgtab_baseline(table_name => l_table_name,table_owner => l_table_owner,sql_handle => l_sql_handle);
